...
<%
	...
	int a_no = MessageDao.getInstance().returnAno(proceeding_no);
	int msg_rno = MessageDao.getInstance().getMemNo2(a_no);
 	int msg_sno = 0; //보내는회원
 	
	/*
	int msg_rno = 0; //받는 회원번호
	int proceeding_no = 0; //진행중인 프로모션 번호 */
	...
	if (user instanceof Creator) {
		...
		msg_sno = creator.getMemNo(); //보내는 회원번호(크리에이터 회원번호)
...
<body>
	...
	<div class="all_section_comu">
		<form id="frm" name="frm" class="form_span" method="post" action="comu_dialog.jsp">
			<br />
			...
			<div>
				...
				<br />
				<br />
				<div class="font_caution_comu">※ 컨텐츠 계약시 파트너와 전화 통화를 하신 후 거래를 진행해 주시기 바랍니다.</div>
				<br />
				...
				<br /> <input class="content_msg_receive" name="msg_content"/> &nbsp;
					<input type="hidden" name="c_no" value="<%=a_no%>"/>
