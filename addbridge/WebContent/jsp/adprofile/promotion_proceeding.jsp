...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	List<Proceeding> pList = (List<Proceeding>)request.getAttribute("Proceeding");
...
function check(index){
	if( checked == false){
		...
		window.location.href="<%=request.getContextPath()%>/MainControl?cmd=pro-complete&p_no=<%=p_no%>";
	}
}
...
function sendMsg() {
	...
	winObj.location.href = "<%=request.getContextPath()%>/jsp/adprofile/ad_comu_dialog.jsp?c_no=<%=pList.get(0).getC_no()%>&proceeding_no=<%=pList.get(0).getProceeding_no()%>";  
}
...
<br/>
   ...
   <div  class="site_proceeding">
   		...
   		<div> <textarea class="textarea_site" required autocomplete="off" readonly="readonly" style="resize: none; overflow: scroll;">  <%=pList.get(0).getP_info().replaceAll("<(/)?([a-zA-Z]*)(\\s[a-zA-Z]*=[^>]*)?(\\s)*(/)?>", "") %></textarea></div>
   </div><br/><br/>
   
   <div class="site_proceeding">
   		<div class="textarea_color"> 제작 컨텐츠 정보</div>
   		<div> <textarea class="textarea_site" required autocomplete="off" readonly="readonly" style="resize: none; overflow: scroll;"> <%=pList.get(0).getP_content().replaceAll("<(/)?([a-zA-Z]*)(\\s[a-zA-Z]*=[^>]*)?(\\s)*(/)?>", "") %> </textarea></div>
   </div><br/><br/><br/><br/><br/>
   <div class="site_proceeding">
